...
export function renderAddProduct(container, params = {}) {
  ...
  function renderImeis() {
    ...
    if (!list) return;
    list.innerHTML = itemBarcodes.map((imei, index) => `
      <span class="chip flex-align-center" style="background:var(--color-bg-alt); padding:5px 10px; border-radius:15px; display:inline-flex; align-items:center; gap:5px; border:1px solid var(--color-border); font-size:12px;">
        ${escapeHtml(imei)}
  ...
  document.getElementById('pf-imei-input')?.addEventListener('keypress', (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    if (!payload.name || !payload.sku || (!payload.barcode && payload.itemBarcodes.length === 0)) {
      showToast('Product Name, SKU, and at least one barcode/IMEI are required', 'warning');
      return;
    }
