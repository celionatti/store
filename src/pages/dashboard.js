    ...
    if (statsGrid) {
      ...
      if (revCanvas) {
        if (revenueChart) {
          ...
          initRevenueChart(revCanvas, data.revenueByMonth);
        }
      }

      function initRevenueChart(canvas, dataPoints) {
                ...
                <tbody>
                  ${lowStock.map(p => `
                    <tr>
                      ...
                      <td data-label="SKU" class="text-muted">${p.sku}</td>
                      <td data-label="Stock">${p.quantity}</td>
                      <td data-label="Reorder">${p.reorderLevel}</td>
                      <td data-label="Status">${p.quantity <= 0
